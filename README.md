...
PaddleOCR is a first-class and important engine in this skill, not an optional afterthought. Route A uses local PaddleOCR 3.7 or newer with PP-OCRv6 medium detection and recognition (`PP-OCRv6_medium_det` + `PP-OCRv6_medium_rec`) in one project-managed environment and one pipeline instance; on macOS it is compared with Apple Vision. Route B uses user-supplied official PaddleOCR AI Studio `.md` and `.json` results as the PaddleOCR candidate without duplicating them locally; on macOS they are likewise compared with Apple Vision. On non-macOS, Codex first obtains the route's PaddleOCR candidate, states that Apple Vision is unavailable, and asks whether the user wants another available OCR backend for comparison. The [current official model table](https://github.com/PaddlePaddle/PaddleOCR/blob/main/docs/version3.x/pipeline_usage/OCR.en.md) lists 59.4 MB plus 73.3 MB for the two local weights, about 0.13 GB combined; the reference Mac's current model cache is about 133 MB. Reserve about 1 GB of disk for the Python environment, runtime files, and cache. Runtime RAM is separate and varies with image size, batch size, and process count.

For non-sensitive PDFs only, you may upload the file to the [official PaddleOCR AI Studio](https://aistudio.baidu.com/paddleocr), use its official online flagship analysis, download the resulting `.md` and `.json`, and tell Codex their paths. If you provide those results before processing begins, Codex uses them directly as the PaddleOCR candidate and does not install local PaddleOCR for that task. On a Mac, Apple Vision supplies the comparison candidate. Record external provenance, file hashes, and page mapping. Do not upload private, confidential, or unauthorized material, and do not describe an online result as a local PaddleOCR run.
...
PaddleOCR 是本 Skill 非常重要的一等引擎，不是可有可无的补充。路线 A 要求 PaddleOCR 3.7 或更高版本，并在一个项目管理的环境和一个 pipeline 实例中使用 PP-OCRv6 medium 检测与识别（`PP-OCRv6_medium_det` + `PP-OCRv6_medium_rec`）；在 macOS 上与 Apple Vision 对照。路线 B 直接使用用户提供的官方 PaddleOCR AI Studio `.md` 和 `.json` 作为 PaddleOCR 候选，不会仅为重复结果而安装或运行本地 PaddleOCR；在 macOS 上同样与 Apple Vision 对照。在非 macOS 上，Codex 会先取得所选路线的 PaddleOCR 候选，说明 Apple Vision 不可用，再询问用户是否要增加另一个当前可用的 OCR 后端。[当前官方模型表](https://github.com/PaddlePaddle/PaddleOCR/blob/main/docs/version3.x/pipeline_usage/OCR.en.md)列出的两个本地模型文件分别约为 59.4 MB 和 73.3 MB，合计约 0.13 GB；参考 Mac 当前的模型缓存约为 133 MB。建议为 Python 环境、运行文件和缓存预留约 1 GB 磁盘空间。运行时内存另行计算，会受图片尺寸、batch 大小和进程数影响。

对于不敏感的 PDF，可以上传到[官方 PaddleOCR AI Studio](https://aistudio.baidu.com/paddleocr)，调用官方在线旗舰分析，下载生成的 `.md` 和 `.json` 后把路径告诉 Codex。如果在处理开始前已经提供这些结果，Codex 会直接把它们作为 PaddleOCR 候选，本次任务不需要安装本地 PaddleOCR；在 Mac 上，由 Apple Vision 提供对照候选。仍需记录外部来源、文件哈希和页面对应关系。不要上传私人、机密或未经授权的内容，也不要把在线结果描述成本地 PaddleOCR 运行结果。
